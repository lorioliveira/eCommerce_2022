...
public class ProdutoHelper implements IViewHelper {
	...
	public EntidadeDominio getEntidade(HttpServletRequest request) {
     ...
     // salva a data atual na tabela de Roupa
		...
		if (("CONSULTAR").equals(operacao)) {
			...
			produto.setId(id);
			produto.setDt_cadastro(dataAtual);
	...
	public void setView(Resultado resultado, HttpServletRequest request, HttpServletResponse response)
		...
		if (("CONSULTAR").equals(operacao)) {
			...
			if (resultado.getMensagem() == null || resultado.getMensagem().equals("")) {
				request.getRequestDispatcher("JSP/consultar-produtos.jsp").forward(request, response);
			}
			else {
				// se houver, mostra as mensagens de ERRO com botão para voltar a tela anterior
				// Guarda a mensagem que veio da Strategy na variável para que 
			...
			if (resultado.getMensagem() == null || resultado.getMensagem().equals("")) {
				...
				if(alteraProduto.contentEquals("0")) {
					...
					// Redireciona para o arquivo .jsp
					request.getRequestDispatcher("JSP/consultar-produtos2.jsp").forward(request, response);
				}
			} 
			else {
				// se houver, mostra as mensagens de ERRO com botão para voltar a tela anterior
